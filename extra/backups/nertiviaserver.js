...
commands=["help","commands","profile","pay","balance","mine"],
ownerCommands=["save database","eval","restart","wipe","setbal"],
adminCommands=["botadmin","say"]
/* /\ End /\ */
...
function command(values){
  let message = values.message,commandValue = values.command;
    if (message===eprefix+commandValue||message===prefix+commandValue)return true;
}
function commandStartsWith(values){
  let message = values.message,commandValue = values.command;
    if (message===eprefix+commandValue||message===prefix+commandValue)return true;
}
  ...
  if (!message.content)return;
    ...
    if (message.content === eprefix+"ping") {
      ...
      if (message.content === eprefix+"commands") {
        message.reply(`Commands: ${commands.toString().replace(/,/g,", ")}`)
    }
...
if (command(message.content,"self setup")) {
  ...
  function profileCard(username){
    ...
    Admin: ${(database.admin.includes(message.author.id)||database.owner.includes(message.author.id)) ? "Yep" : "No"}
    `;
...
*/
  ...
  if (database.admin.includes(message.author.id)||database.owner.includes(message.author.id)){
    ...
    message.delete();message.channel.send("" + (message.content.split(" ").slice(2)).join(" "))
    }
    
        if (message.content === eprefix+"admin commands") {
        message.reply(`Admin Commands: ${adminCommands.toString().replace(/,/g,", ")}`)
    }
  ...
  if (database.owner.includes(message.author.id)){
        ...
        message.reply(`Owner Commands: ${ownerCommands.toString().replace(/,/g,", ")}`)
    }
...
if (message.content==(eprefix+"restart")) {
  ...
  try{
    ...
    if (message.content.startsWith(eprefix + 'setbal')) {
    var args = message.content.split(" "),
        user = message.mentions.users.first(),
        amount=Math.round(args[3]);
    if (!user){return message.reply("No user mentioned.")}
...
client.login("Njc3MzM2NzYwMjAxOTMwNzUyMA.SOEXY0hXTpeCf3nY8f4vFtejRkxELwsaBjLlA86RnlU")
