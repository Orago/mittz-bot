...
client.on("message", async (message) => {
  ...
  if (database.guild[message.guild.id].prefix){
    ...
    if (database.guild[message.guild.id].commandschannel){
      
      if (database.guild[message.guild.id].commandschannel !== message.channel.id){
        if(!message.member.hasPermission('ADMINISTRATOR')||message.author.id!==message.guild.ownerID){
          message.delete(); 
          return message.author.send("**(Server: `"+message.guild.name+")`** Please use this in the commands channel <#"+database.guild[message.guild.id].commandschannel+">.\n`If you think this isn't correct, then please ask the owner to change the commands channel.`")}}}
...
message.channel.send({ embed: exampleEmbed });
  ...
  var data="Crap";
    ...
    if (!username){message.channel.send("No username given.")}
await fetch(`https://beta.meown.tk/api?method=user&search=${username}`)
    .then(res => res.json())
    .then(json => data=json);
  if (data.response){
...
var user = data.response;
  ...
  thumbnail: {
    ...
    {name:'Creation Date', value: new Date(Number(user.creation_date))},
...
ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  var amount = ((Math.floor(Math.random() * (3 - -4)) + -3)*0.01);
  if (!database.user[message.author.id]){return message.channel.send(`<@${message.author.id}> No profile found.`);}
  add_coin(message.author.id,amount)
    return message.channel.send(`<@${message.author.id}> Mined ${amount} coins.\n You now have ${database.user[message.author.id].coins}`);
  }
  function add_coin(id,amount){
    if (!database.user[id]){return console.log(`<@${id}> No profile found.`);}
    else{
      database.user[id].coins+=amount;
    }
  }
    function remove_coin(id,amount){
      amount = amount * -1;
    add_coin(id,amount)
    }
  
  function profile_card(user_type){
    if (!database.user[user_type.id]){return message.channel.send(`<@${user_type.id}> No profile found.`);}
        const canvas = Canvas.createCanvas(700, 250);
       const ctx = canvas.getContext('2d');
       Canvas.loadImage(database.user[user_type.id].background).then((background) => {
               ...
               Canvas.loadImage('https://www.stickpng.com/assets/images/585e4beacb11b227491c3399.png').then((lb) => {
                  Canvas.loadImage('https://cdn.discordapp.com/attachments/660390332772646922/744094457317818388/discordowner.svg').then((owner) => {
                   Canvas.loadImage('https://discordapp.com/assets/ccebe0b729ff7530c5e37dbbd9f9938c.svg').then((rich) => {
                     ...
                     if (database.user[user_type.id].badges.includes("rich")) {
                       ctx.drawImage(rich, 320, 147, 40, 40);}
                     if (message.author.id===message.guild.ownerID) {
                       ctx.drawImage(owner, 185, 50, 55, 40);}
                     ctx.font = '40px sans-serif';
                     ctx.fillStyle = database.user[user_type.id].color;
                     ctx.fillText(`${user_type.tag}`, 240, 90);
                     ctx.font = '25px sans-serif';
                     ctx.fillText(`${database.user[user_type.id].xp}xp`, 270, 125);
                     ctx.fillText(`${database.user[user_type.id].coins} coins`, 270, 170);
                     ctx.fillText(`Bio: ${database.user[user_type.id].description}`, 77, 229);
...
if (message.content.startsWith (eprefix+'votekick')) {
 ...
 if (votes[message.guild.id][user.id] == undefined||votes[user.id] == null){
  ...
  var voters = votes[message.guild.id][user.id][votes[message.guild.id][user.id].length-1];
  //message.channel.send(`<@${user.id}> | ${JSON.stringify(votes)} | ${message.guild.members.cache.filter(member => !member.user.bot).size}`);
	message.channel.send(`You voted to kick <@${user.id}> | Current Voters: [ <@${voters}> ]`);
  message.channel.send(voters.length)
  if (voters.length>database.guild[message.guild.id].votekick_min)
...
if (!database.guild[message.guild.id]){return setup_message}
  ...
  if (message.content.startsWith(eprefix+"createrole old")) {
...
client.users.fetch(member.id).then((user) => {
  ...
  if (message.content==(eprefix+"set-logs")) {
     ...
     if (message.channel.type !== "text") {return message.channel.send('Please do this in a server with `'+client.user.username+'`')}
      ...
      database.guild[message.guild.id].logschannel=message.channel.id;
      fs.writeFileSync(database_location, JSON.stringify(database, null, 2));
    } else
    if (message.content==(eprefix+"set-commands")) {
     if (message.channel.type !== "text") {return message.channel.send('Please do this in a server with `'+client.user.username+'`')}
      message.channel.send("Commands channel channel changed");
      database.guild[message.guild.id].commandschannel=message.channel.id;
      fs.writeFileSync(database_location, JSON.stringify(database, null, 2));
    }else 
    if (message.content==(eprefix+"set-welcome")) {
     if (message.channel.type !== "text") {return message.channel.send('Please do this in a server with `'+client.user.username+'`')}
      message.channel.send("Welcome channel changed");
      database.server[message.guild.id].greetingschannel=message.channel.id;
      fs.writeFileSync(database_location, JSON.stringify(database, null, 2));
...
client.on("messageDelete", async message => {
  ...
  if (database.servers.includes(message.guild.id)){
  if (database.guild[message.guild.id].logschannel){logchannel=client.channels.cache.get(database.guild[message.guild.id].logschannel);}}
  ...
  client.on("guildMemberRemove", member =>{
    if (database.servers.includes(member.guild.id)){
    if (database.greetingschannel[member.guild.id]){greetingschannel=client.channels.cache.get(database.greetingschannel[member.guild.id]);}}
