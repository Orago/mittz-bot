...
client.on("channelDelete", async (channel, message) => {
  var logs = message.guild.channels.cache.find(
    channel => channel.name === "〔🤖｜bot-logs〕"
  );
  ...
  const cembed = new Discord.MessageEmbed()
    ...
    .setColor("RANDOM")
    .setDescription(
      `A **${channel.type} channel**, by the name of **${channel.name}**, was just deleted!`
    )
...
client.on("message", message => {
  ...
  if (message.content.startsWith(eprefix + "ping")) {
    try {
      message.channel.send("Pinging...").then(msg => {
        // Make sure the async is written, top of the client.on("message", ...)
  ...
  if (message.content === "invites") {
    ...
    var userInvites = message.guild
      .fetchInvites()
      .then(invites => invites.find(invite => invite.inviter.id === userId));
    var useAmount = userInvites.uses;
    if (useAmount === undefined) {
      message.channel.send(`${message.author.username} has 0 invites`);
    } else {
      message.channel.send(
        `${message.author.username} has ${useAmount} invites`
      );
    }
  }

  if (
    message.content == eprefix + "help" ||
    message.content == eprefix + "commands" ||
    message.content == eprefix + "about"
  ) {
    const embed = new Discord.MessageEmbed()
      .setThumbnail(client.user.displayAvatarURL)
      .setTitle("__Mittz Information__")
      .setDescription(
        "Hello! <:gold:733213975705026620> I'm " +
          botname +
          ".\nI can do lots of things l-like... Oh! Be a DJ!, send cat pictures!, clear a bit of the chat, and run a failing economy system.  Well... There isn'nt much else i can do... Use `" +
          eprefix +
          "commandlist` to see what other things i can do."
      )
  ...
  if (message.content.startsWith(eprefix + "subscribers")) {
    ...
    if (!role.includes("https://www.youtube.com/channel/")) {
      message.channel.bulkDelete(1);
      return message.reply("not a valid link");
    }
    var url = role.lastIndexOf("l/");
    var id = role.slice(url + 2, Infinity);
  ...
  if (message.content.startsWith(eprefix + "prof")) {
    var registered = false;
    if (!server.profiles.includes(message.author.id)) {
    ...
    if (!server.profiles.includes(person.id)) {
      return message.channel.send(
        `Please ask the user @ ${person.username} to setup ` +
          client.user.username +
          `economy with ` +
          "`" +
          eprefix +
  ...
  if (message.content.includes("owo")) {
    if (!server.servers.includes(message.guild.id))
      return message.channel.send(
        "Please ask the server owner to setup owo's with `" +
          eprefix +
          "guild setup`"
      );
    if (!server.profiles.includes(message.author.id))
      return message.channel.send(
        "Please setup owo's with `" + eprefix + "self setup`"
      );
  ...
  if (message.content.includes("uwu")) {
    if (!server.servers.includes(message.guild.id))
      return message.channel.send(
        "Please ask the server owner to setup uwu's with `" +
          eprefix +
          "guild setup`"
      );
    if (!server.profiles.includes(message.author.id))
      return message.channel.send(
        "Please setup uwu's with `" + eprefix + "self setup`"
      );
  ...
  if (message.content == eprefix + "self setup") {
    if (!server.servers.includes(message.guild.id))
      return message.channel.send(
        "Please ask the server owner to setup kawaii counter with `" +
          eprefix +
          "guild setup`"
      );
    if (server.profiles.includes(message.author.id))
      return message.channel.send(
        ":closed_lock_with_key: **Whoops!** You already have a profile!"
      );
    message.channel
      .send("**:unlock: Beep Boop Beep! We're setting up your profile!**")
      .then(msg => {
        ...
        server.background[message.author.id] =
          "https://convertingcolors.com/plain-2C2F33.svg";
        ...
        setTimeout(() => {
          msg.edit(
            ":lock: **Your user has been added.** View the leaderboard with `kawaii leaderboard`."
          );
        }, 1500);
      });
  }
  if (message.content == eprefix + "guild setup") {
    if (server.servers.includes(message.guild.id))
      return message.channel.send(
        ":closed_lock_with_key: **Whoops!** This guild is already set up!"
      );
    message.channel
      .send("**:unlock: Beep Boop Beep! We're setting up your profile!**")
      .then(msg => {
        ...
        setTimeout(() => {
          msg.edit(
            ":lock: **Your guild has been added.** View the leaderboard with `kawaii leaderboard`."
          );
        }, 1500);
      });
  }
  ...
  if (message.content == eprefix + "homepage") {
    const embed = new Discord.MessageEmbed()
      ...
      .setAuthor(
        "Mittens Homepage",
        "https://cdn.glitch.com/65f81ac1-5972-4a88-a61a-62585d79cfc0%2F8bitpc%20vaporwave%20blue%20cat.png?v=1594354853240"
      )
      .setColor(0x00ae86)
      .setDescription(
        "This is the homepage for all of the projects made by the same creator of this very bot!"
      )
      .setFooter(
        "Created by Orago",
        "https://cdn.glitch.com/65f81ac1-5972-4a88-a61a-62585d79cfc0%2Fboxie-2048px.png?v=1594354728664"
      ...
      .setThumbnail(
        "https://cdn.glitch.com/65f81ac1-5972-4a88-a61a-62585d79cfc0%2Finbreadspread.png?v=1594354874436"
      )
      .setTimestamp()
  ...
  if (message.content.startsWith(eprefix + "play")) {
    if (
      message.guild.id == 396838715894530068 &&
      message.channel.id != 741545026055569438
    ) {
      return;
    }
    ...
    if (voiceChannel && voiceChannel.id !== "741450921690791996") {
      return message.channel.send("please join the music channel");
    }
    ...
    if (!type)
      return message.reply(
        "The currently available radio's are lofi, vaporwave, jpop, and kpop.\nEx. " +
          eprefix +
          " play lofi"
      ); // Checks if the `amount` parameter is given
    if (type == "lofi") {
      message.reply("you are now listening to the " + type + " radio.");
      ...
      voiceChannel.join().then(connection => {
        //const stream = ytdl('http://hyades.shoutca.st:8043/stream', { filter: 'audioonly' });
        const dispatcher = connection.play("http://radio.plaza.one/mp3", {
          volume: 1
        }); //connection.play(stream);
        dispatcher.on("finish", () => voiceChannel.leave());
      });
      ...
      message.reply(
        "you are now listening to the " +
          type +
          " radio. This station is hosted by https://listen.moe/"
      );
      voiceChannel.join().then(connection => {
        //const stream = ytdl('http://hyades.shoutca.st:8043/stream', { filter: 'audioonly' });
      ...
      voiceChannel.join().then(connection => {
        //const stream = ytdl('http://hyades.shoutca.st:8043/stream', { filter: 'audioonly' });
        const dispatcher = connection.play("https://listen.moe/kpop/fallback", {
          volume: 1
        }); //connection.play(stream);
        dispatcher.on("finish", () => voiceChannel.leave());
      });
  ...
  if (message.content === eprefix + "radio source") {
    var embed = new Discord.MessageEmbed()
      ...
      .addField(
        "View Guild Insights",
        message.member.hasPermission("VIEW_GUILD_INSIGHTS"),
        true
      )
      .setColor("#5CC5FF")
      //.setDescription(`hey`);
  ...
  if (message.content === eprefix + "permissions") {
    var embed = new Discord.MessageEmbed()
      ...
      .addField(
        "Priority Speaker",
        message.member.hasPermission("PRIORITY_SPEAKER"),
        true
      )
      .addField(
        "Mute Members",
      ...
      .addField(
        "Use VAD (voice activity direction)",
        message.member.hasPermission("USE_VAD"),
        true
      )
      .addField(
        "Add Reactions",
      ...
      .addField(
        "Send TTS Messages",
        message.member.hasPermission("SEND_TTS_MESSAGES"),
        true
      )
      .addField(
      ...
      .addField(
        ...
        message.member.hasPermission("READ_MESSAGE_HISTORY"),
        true
      )
      .addField(
        "Mention Everyone",
      ...
      .addField(
        "Use External Emojis",
        message.member.hasPermission("USE_EXTERNAL_EMOJIS"),
        true
      )
      .addField(
        "View Channel",
  ...
  if (
    ...
    if (message.content.startsWith(eprefix + "clear")) {
      ...
      message.channel.messages.fetch({ limit: amount }).then(messages => {
        ...
        if (amount > 1) {
          message.channel.send(
            ":white_check_mark: " + amount + " messages have been deleted"
          );
        } else {
          message.channel.send(":white_check_mark: 1 message has been deleted");
        }
      });
    }
            ...
            .then(collected => {
              if (collected.first().emoji.name == yes) {
                member
                  ...
                  .then(() => {
                    user.send(
                      "You have been kicked by <@" + message.author.id + ">"
                    );
                  ...
                  .catch(err => {
                    ...
                    user.send(
                      "<@" + message.author.id + "> has attempted to kick you"
                    );
                    console.error(err);
                  });
              } else message.channel.send("Operation canceled.");
...
client.on("guildCreate", guild => {
  ...
  channel.send(
    "Thank you for inviting me, I am hoping to help whenever I can."
  );
  console.log(
  ...
  client.user.setPresence({
    activity: {
      name: `${eprefix} help | ${client.guilds.cache.size} guilds`,
      type: "STREAMING",
      url: "https://www.youtube.com/watch?v=P4i-VYcrEuc"
    },
    status: "idle"
  });
});
